    ...
    if (isset($navData['parents'][$parent_id])) {
        ...
        foreach ($navData['parents'][$parent_id] as $navItem) {
            if ($navData['items'][$navItem]['visible_' . $type] == 1) {
                ...
                } elseif ($navData['items'][$navItem]['template'] == 'i') {
                    $nav_url = $root_url . '/' . get_page_data($navData['items'][$navItem]['url'], 'url');
                    $nav_target = "";
                } elseif ($navData['items'][$navItem]['template'] == 'f') {
                    $nav_url = "#";
                    $nav_target = "";
                } elseif ($navData['items'][$navItem]['url'] == 'index/' || $navData['items'][$navItem]['url'] == 'index') {
                    $nav_url = $root_url . '/';
                    $nav_target = "";
                } else {
    ...
    if (!is_null($this_page)) {
        while ($current_level != 0) {
            ...
            if ($data['visible'] != 0) {
                ...
                } elseif ($data['published'] == 0 || $data['template'] === 'f' || $data['template'] === 'e') {
